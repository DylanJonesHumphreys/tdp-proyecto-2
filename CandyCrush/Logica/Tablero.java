...
public class Tablero implements TableroJuego{
		...
		for (int i=0; i < reg.length; i++) {
			if (reg[i] != null) {
				...
				switch (regla) {
					...
					case "R4": {
						Regla_match_4_o_más r4 = new Regla_match_4_o_más();
		...
		do{
			...
			for(int f = 0; f < filas && !existen_entidades_a_detonar; f++){
				for(int c = 0; c < columnas && !existen_entidades_a_detonar; c++){
					efecto_regla = regla.verificar_regla(entidades, entidades[f][c], entidades[f][c]);
	...
	protected EfectosDeTransicion calcular_efectos_por_intercambio(Entidad entidad_origen, Entidad entidad_destino){
		...
		if (entidad_origen.detona_de_a_dos(entidad_destino)) {
			...
			verificar_match(efecto_transicion, entidad_origen, entidad_destino);
		}
	...
	protected void verificar_match(EfectosDeTransicion efecto_transicion, Entidad entidad_con_quien_hizo_intercambio, Entidad entidad_intercambiada_para_formar_match){
		for(Regla r: reglas_de_match){
			EfectoRegla efecto_regla=r.verificar_regla(entidades, entidad_con_quien_hizo_intercambio, entidad_intercambiada_para_formar_match);
			agregar_a_efecto_detonacion(efecto_regla,efecto_transicion);
			agregar_a_incorporar(efecto_regla,efecto_transicion);
		}
	}
	...
	private void agregar_a_efecto_detonacion(EfectoRegla efecto_regla, EfectosDeTransicion efecto_transicion){
		Collection<Entidad> entidades_a_detonar = efecto_regla.get_entidades_verificadas_a_detonar();
	...
	private void pasar_a_efecto_transicion(Collection<Entidad> entidades_a_detonar, EfectosDeTransicion efecto_transicion){
	...
	private void agregar_a_incorporar(EfectoRegla efecto_regla, EfectosDeTransicion efecto_transicion){
		 ...
		 * de entidades_a_reemplazar, pues su reemplazo ya es conocido (la entidad a_incorporar)
		 */
	...
	public void imprimirTablero(){		//TODO: borrar antes de entrega
		Entidad entidad;
		for(int f=0;f<filas;f++){
			for(int c=0; c<columnas; c++){
				entidad = entidades[f][c];
				if(entidad instanceof Caramelo){
					System.out.print("C"+int_a_color(entidad.get_color()));
				}
				else if(entidad instanceof Gelatina){
					System.out.print("J"+int_a_color(entidad.get_caramelo().get_color()));
				}
				else if(entidad instanceof Glaseado){
					System.out.print("GL ");
				}
			}
			System.out.println();
	...
	private String int_a_color(int numero_color){
		if(numero_color==1)
			return "V ";
		else if(numero_color==2)
			return "L ";
		else if(numero_color==3)
			return "A ";
		else if(numero_color==4)
			return "R ";
		else if(numero_color==5)
			return "B ";
		else if(numero_color==6)
			return "P ";
		else
			return "Error";
	}
