...
import Entidades.CarameloDummy;
...
public class Tablero implements TableroJuego {
	...
	public Tablero(Juego juego) {
		...
		filas = 0;
		columnas = 0;
		reglas_de_match = new LinkedList<Regla>();
	}
	...
	public void agregar_regla(Regla regla){
		reglas_de_match.add(regla);
	}

	////// Operaciones para Tablero Entidad (Tablero <-- Entidad)
	...
	public void mover_jugador(int direccion) {
		switch(direccion) {
			case Juego.ABAJO:{mover_jugador(fila_jugador + 1, columna_jugador); break;}
			case Juego.ARRIBA:{mover_jugador(fila_jugador - 1, columna_jugador); break;}
			case Juego.IZQUIERDA:{mover_jugador(fila_jugador, columna_jugador - 1);break;}
			case Juego.DERECHA:{mover_jugador(fila_jugador, columna_jugador + 1);break;}
		}
	}
	...
	public void intercambiar_entidades(int direccion) {
		switch(direccion) {
			case Juego.ABAJO:{intercambiar_entidades_y_transicionar(fila_jugador + 1, columna_jugador);break;}
			case Juego.ARRIBA:{intercambiar_entidades_y_transicionar(fila_jugador - 1, columna_jugador);break;}
			case Juego.IZQUIERDA:{intercambiar_entidades_y_transicionar(fila_jugador, columna_jugador - 1);break;}
			case Juego.DERECHA:{intercambiar_entidades_y_transicionar(fila_jugador, columna_jugador + 1);break;}
		}
	}
	

	////Aplicacion de transiciones: control de matches, detonaciones, caidas y objetivos.
	...
	private void intercambiar_entidades_y_transicionar(int fila_destino, int columna_destino) {
		...
		if (en_rango(fila_destino, columna_destino)) {	
			...
			if (entidad_origen.es_posible_intercambiar(entidad_destino)) {
				ejecutar_intercambio(entidad_origen, entidad_destino);
			}
		}
	}
	
	private void ejecutar_intercambio(Entidad entidad_origen, Entidad entidad_destino){
		EfectosDeTransicion efecto_intercambio;
		int fila_origen = fila_jugador;
		int columna_origen = columna_jugador;
		int fila_destino = entidad_destino.get_fila();
		int columna_destino = entidad_destino.get_columna();
		...
		if(efecto_intercambio.existen_entidades_a_detonar()){
			procesar_entidades_a_detonar(efecto_intercambio);
		}
		else{
			deshacer_intercambio(fila_origen,columna_origen,fila_destino,columna_destino);
		}
	}
	
	private EfectosDeTransicion calcular_efectos_por_intercambio(Entidad entidad_origen, Entidad entidad_destino){
	...
	private void cambiar_posicion_jugador(int nueva_fila, int nueva_columna) {
		fila_jugador = nueva_fila;
		columna_jugador = nueva_columna;
	}
	
	private void deshacer_intercambio(int fila_origen, int columna_origen,int fila_destino, int columna_destino){
		Entidad entidad_origen = entidades[fila_origen][columna_origen];
		Entidad	entidad_destino = entidades[fila_destino][columna_destino];
		cambiar_posicion_jugador(fila_origen, columna_origen);
		entidad_origen.intercambiar(entidad_destino);
	}
	
	private void procesar_entidades_a_detonar(EfectosDeTransicion efecto_intercambio){
	...
	protected void agregar_entidades_nuevas(Set<Entidad> entidades_a_incorporar) {
	...
	/////metodos relacionados a aplicar la caida
	...
	private Caramelo reemplazar_por_caramelo_provisorio(Caramelo caramelo_a_reemplazar) { 
		...
		CarameloDummy caramelo_provisorio = new CarameloDummy(this, 
